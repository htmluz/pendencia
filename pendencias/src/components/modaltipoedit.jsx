...
function ModalTipoEdit( { fecharModal, tipoEdit }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axiosPrivate.put(`/tipos/edit/${tipoEdit}`, formData, {
                headers: {
            ...
            <div className='bg-white w-1/4 mt-20 ml-auto mr-auto rounded p-4 pb-0 shadow-modal'>
                    <div className='relative flex flex-col w-full pointer-events-auto p-5 pb-8'>
                        <div className='font-Inter font-bold text-lg flex justify-between align-top text-right mb-2 pb-2'>
                            <h5 className="select-none cursor-default font-normal"><b>Editar Tipo</b></h5>
                        ...
                        <form onSubmit={handleSubmit} className="font-system font-semibold" method='PUT'>
                            ...
                            <div className="mt-3">
                                <button type="submit" className='cursor-default transition-colors  bg-[#187bcd] hover:bg-[#1167b1] rounded px-3 py-2 w-full text-white'>Alterar Tipo</button>
                            </div>
                        </form>
                    </div>
            </div>
